...
const UK_PHONE_REGEX = /^(?:\+44\s?|0)\s*(?:\d\s*){9,10}$/;
...
publicWidget.registry.UkCheckoutCustom = publicWidget.Widget.extend({
    ...
    _onSubmit(ev) {
        ...
        if (phone && phone.value && !UK_PHONE_REGEX.test(phone.value.trim())) {
            phone.setCustomValidity('Please enter a valid UK phone number.');
